  ...
  $.widget( "epilgrim.sessionTimeoutHandler", {
      ...
      getSessionTimeLeft: function (event, data) {
        ...
        $.ajax({
          ...
          success: function (time){
            ...
            if (timeLeft > this.options.warnWhenLeft) {
    ...
    _createModal: function(){
      ...
      this.modal.dialog({
        ...
        open: function(event, ui) {
          $(".ui-dialog-titlebar-close", this.modal).hide();
        },
        buttons: {
    ...
    _dialogHandler: function(){
      if(this.options.checkTimeBeforePopup){
        this._trigger('getSessionTimeLeft', null, {plugin: this, updateSession: false, callback: function(timeLeft){
          if (timeLeft <= this.options.warnWhenLeft){
            this.modal.dialog('open');
          } else {
            this.initializeTimers(timeLeft);
          }
        }});
      }else{
        this.modal.dialog('open');
